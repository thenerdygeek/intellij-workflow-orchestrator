...
describe('AgentMessage — real-time streaming markdown', () => {
  ...
  it('renders paragraph text as plain text nodes (no per-word span wrapping)', () => {
    const { container } = render(
  ...
  it('renders text in code blocks without extra span wrappers', () => {
    const code = 'Before code\n\n```python\ndef foo(): return 42\n```\n\nAfter code';
    const { container } = render(
      <AgentMessage message={streamingMessage(code)} isStreaming={false} />,
    );
    // Paragraph text is accessible as textContent
    const paragraphs = Array.from(container.querySelectorAll('p'));
    const paragraphTexts = paragraphs.map(p => p.textContent);
    expect(paragraphTexts).toContain('Before code');
    expect(paragraphTexts).toContain('After code');
  });

  it('renders inline code content without extra span wrappers', () => {
    const { container } = render(
      <AgentMessage
