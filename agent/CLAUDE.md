...
| **python_runtime_exec** | 2 | run_tests (pytest via PytestActions), compile_module (python -m py_compile) — registered only when Python plugin present. Pytest output reconciles verbose PASSED/FAILED/ERROR counts against the summary line and emits a heuristic warning when the suite reports passes with near-zero duration and no stdout. |
