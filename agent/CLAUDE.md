...
Deferred, registered under category `"Utilities"` in `AgentService.registerAllTools()`. Actions: `start | list | stop | status`. Sources: `source=shell`, (Phase 2) `source=bamboo`, (Phase 3) `source=pull_request`, and (Phase 4) `source=jira_ticket` / `source=jira_sprint`. The `start` branch is a `when(source)` factory dispatching to `startShell` / `startBamboo` / `startPullRequest` / `startJiraTicket` / `startJiraSprint`. Shell parameters: `command` (shell command to run), `filter` (regex — **case-sensitive by default; `(?i)` prefix for case-insensitive**). Bamboo parameters: `plan_key` (required), `branch` (omit for main plan), `level` (`build`|`stage`|`job`, default `build`), `stage_name` (required for stage/job), `job_name` (required for job). Pull-request parameters: `pr_id` (numeric, required), `aspects` (comma-separated subset of `state,reviews,comments`; default all three), `repo_name` (optional slug override). Jira-ticket parameters: `ticket_key` (e.g. `PROJ-123`, required). Jira-sprint parameters: `board_id` (numeric, resolves active sprint via `getAvailableSprints`) and/or `sprint_id` (numeric, polls directly; at least one required). Common: `description` (label), `monitor_id` (for stop/status). `allowedWorkers = {ORCHESTRATOR, CODER}`. `validateStart()` (shell), `validateBambooStart(planKey, level, stageName, jobName)` (bamboo), `validatePullRequestStart(prIdRaw, aspects)` (pull_request), `validateJiraTicketStart(ticketKey)` (jira_ticket), and `validateJiraSprintStart(boardIdRaw, sprintIdRaw)` (jira_sprint) are pure companion methods (unit-testable), dispatched by source in `execute`. `renderStatus(handle)` is a pure companion method that formats one monitor's state + buffered matched-event lines (used by `action=status`). On `start`, a UUID-prefixed id is generated and returned (`shell-{8char}` / `bamboo-{8char}` / `pr-{8char}` / `jira-ticket-{8char}` / `jira-sprint-{8char}`). `BambooService` / `BitbucketService` / `JiraService` are resolved via `ServiceLookup` (injected as optional provider lambdas for testability); unconfigured services return graceful errors. A shell monitor auto-removes on process exit, so `stop`/`status` on an already-exited shell monitor returns `No monitor with id …` (expected); Bamboo, PR, and Jira sources run until `monitor stop` / session dispose.
