...
- **Dual LLM provider (Phase 4a, 2026-06-30):** `AgentSettings.llmProvider` selects the back-end. `"sourcegraph"` (default) = the XML-in-content path above. `"anthropic"` = **`AnthropicDirectBrain`** (`:agent/ai/`) calling `api.anthropic.com` natively (`x-api-key`, top-level `system` field, structured `tools:[]`/`tool_use`, adaptive thinking + effort, inline base64 images, proxy-aware OkHttp via the `:web` `IdeProxy`/`IdeTrust` triad). **Option A ("native wire, XML-internal"):** the model's structured `tool_use` is **serialized back to canonical XML** at stream end (`ToolUseXmlSerializer` via `AnthropicSseParser`), so `AssistantMessageParser`, `ApiMessage` persistence, and the dialect-drift machinery are unchanged. Provider choice is exclusive at `BrainFactory.create()` — the native branch is FIRST (before the blank-Sourcegraph guard) and resolves `modelOverride ?: anthropicModel ?: AnthropicModelCatalog.defaultModel()`. The native path **skips `BrainRouter`** (images go inline as base64 blocks, never routed to `/stream`) and pairs the loop + EVERY prompt-build/dialect-guard site (incl. `SubagentRunner`/`MessageStateHandler`, threaded per task) with **`AnthropicNativeProtocol`** (`presentTools→null` — tools live only in the wire `tools:[]` field, never also in the prompt, so the historical double-presentation drift cannot recur). Context windows come from a static `AnthropicModelCatalog` + its `ModelCatalogService` adapter (no `GET /.api/llm/models`). Request carries no sampling params (structural) + one ephemeral `cache_control` on the system block. Spec: `docs/superpowers/specs/2026-06-30-plugin-split-phase4a-design.md`.
...
- **AgentLoop** (`loop/AgentLoop.kt`, ~1471 lines) — Core ReAct loop. Tool call processing, context compaction on overflow, parallel read-only tool execution (via coroutineScope+async), sequential write tool execution. Truncated tool call recovery — detects invalid JSON when finishReason=length, asks LLM to retry with smaller operation. Mid-loop cancellation support. Context overflow replay (compress + retry same request). Doom loop detection before each tool call. Streaming token estimate when usage is null. Plan mode execution guard (blocks tools where `isMutating=true` or `isWriteAction(action)=true`, even if LLM hallucinates them).
- **AgentService** (`AgentService.kt`, ~1549 lines) — Main orchestration service. Manages tool registration, session lifecycle, plan mode state (per-session via `PerSessionAgentState`, accessed by `isPlanModeActive()` / `setPlanModeActive(enabled)`), network-error recovery setup (brainFactory + cachedFallbackChain wiring), context management setup. Builds `AgentLoop` with all callbacks wired. Schema filtering for plan mode (removes write tools + `enable_plan_mode` from tool definitions before LLM call). Dynamic tool definition provider rebuilds system prompt when tool set changes. **Brain construction extracted (Phase 3 cut D):** the per-task brain factory + model-selection precedence live in `brain/BrainFactory.kt` — `AgentService.createBrain()` delegates to `BrainFactory.create()`. The branchy precedence (caller override > saved `AgentSettings.sourcegraphChatModel` > auto-picked `ModelCache.pickBest` > `LlmBrainFactory` fallback) is carved into the pure `suspend BrainFactory.resolveModel(...)`, pinned behaviorally by `BrainModelResolutionTest` (the 2026-05-06 5x-over-billing regression: saved selection must beat pickBest). `BrainFactory` is a fork seam. Still on `AgentService`, deferred to a later cut: `wrapBrainWithRouter` + the shared `ModelCatalogService` holder + `activeAttachmentStore`/`currentBrainModelId` session state. **Background-completion routing extracted (Phase 3 cut F):** `tools/background/BackgroundCompletionCoordinator.kt` owns the `BackgroundPool` completion listener + routing (test-capturer → live-loop steering → idle persist+auto-wake) + the two synthetic-message builders (`buildCompletionSystemMessage` / `buildAutoResumeSyntheticMessage`, now pure & pinned by `BackgroundCompletionCoordinatorTest`). The shared auto-wake substrate (`idleWaker`/`autoWakeGuards`/`autoWakeListener`/`autoWakeIdleSession`/`activeLoopForSession`) **stays on `AgentService`** — used by monitor, background, and delegation wakes — and is injected into the coordinator as the `autoWake` lambda. **Resume preamble away-event sections extracted (Phase 3 cut C, incision 1):** the three "splice persisted away-events into the `[TASK RESUMPTION]` preamble" string-builders in `resumeSession` (background completions, cross-IDE delegation nudges, monitor notifications) are carved into pure `ResumeHelper.formatBackgroundCompletionsSection(...)` / `formatDelegationNudgesSection(...)` / `formatMonitorNotificationsSection(...)` — each returns the section text to append, or `""` when its list is empty so the caller appends nothing. The side-effecting load + per-item consume/clear + log stay inline in `resumeSession`; only the (previously untested) formatting is now behaviorally pinned by `ResumeHelperTest`. **Resume history cleanup transforms extracted (Phase 3 cut C, incision 2):** the two pure list transforms at the top of `resumeSession` — trailing empty/blank-assistant drop (mirrors `ContextManager`/`MessageStateHandler.pruneTrailingEmptyAssistants`) and dialect-drift redaction (per-assistant-turn `DialectDriftDetector.redactDialectMarkers`) — are carved into `ResumeHelper.dropTrailingEmptyAssistants(history)` → `TrailingEmptyDropResult(history, droppedCount)` and `ResumeHelper.redactDialectDriftInHistory(history)` → `DialectRedactionResult(history, redactedCount)` (unchanged turns returned as the SAME instances). The call site keeps the logging + the `dialectDriftDetectedOnResume` flag-raise inline. Pinned by `ResumeHelperTest`. (Removing the now-only-inline `DialectDriftDetector` use made its `AgentService` import unused → import removed AND the `ImportOrdering:AgentService.kt` detekt-baseline entry surgically updated — the recurring import-block-baseline trap.) **TASK_RESUME-hook cancel note extracted (Phase 3 cut C, incision 3):** the `buildString` that composes the resume-cancelled chat note (and quotes the user's dropped follow-up back to them — mirrors fix 56906e668) is carved into pure `ResumeHelper.buildResumeCancelledNote(reason, userText)` (null/blank reason omits the `": …"` clause; null/blank userText omits the quoted block; each userText line prefixed `> `). Pinned by `ResumeHelperTest`. **Delegation-target composition extracted (Phase 3 cut G, incision 1):** the dedup-merge-filter-map at the tail of `computeDelegationTargetsForPrompt` (drop discovered paths already in recents → drop `"missing"` paths → map to `SystemPrompt.DelegationTarget`) is carved into the pure `tools/delegation/DelegationTargetComposer.compose(recents, discovered)`; the call site keeps the settings gate + the two socket probes (`defaultRecentsProvider`/`defaultDiscoveredProvider`) inline and delegates the decision. Pinned by `DelegationTargetComposerTest`. Note: the broader cut-G goal (lifting the `startDelegatedSession`/`resumeDelegatedSession` wrappers into their own service) stays BLOCKED until B/C (`executeTask`/`resumeSession`) are themselves services — the wrappers call those AgentService methods directly — so cut G proceeds by pure-slice incisions, not whole-cluster extraction. **Delivery-failure mapping extracted (Phase 3 cut G, incision 2):** the four near-identical catch-block constructions in `startDelegatedSession` / `resumeDelegatedSession` that map a delivery-time throwable to a `DelegationMessage.Result` (`CancellationException` → `CANCELED` with its message; any other → `FAILED` with message-or-qualified-class-name) collapse into the pure companion `AgentService.delegationResultForDeliveryFailure(e, durationSeconds)` (sibling of `mapLoopResultToDelegationResult`). Callers still own rethrowing the `CancellationException` after delivery and the benign-disconnect short-circuit. Pinned by `DelegatedResultMappingTest`. With the pure slices now harvested (composer + this mapping this session; `isBenignDeliveryDisconnect` / `delegatedIncomingUiMessageOverride` / `mapLoopResultToDelegationResult` in prior cuts), the delegated wrappers' remaining inline logic is genuinely stateful orchestration (counters, channel registration, coroutine launch, teardown) — no clean pure slice left.
- **AgentController** (`ui/AgentController.kt`) — UI entry point. Owns `AgentCefPanel`, routes user messages, handles steering, dispatches JCEF bridge callbacks (tool approval, plan approval, session history, artifact results). Wires `AgentService.executeTask()` with UI callbacks. **Model-picker version ordering extracted (Phase 3 cut):** the local `versionKey` / `versionNums` / `byVersionAsc` functions inside `loadModelList` (strip `-thinking`/`-latest`/8-digit-date suffix → generation key; digit-group numeric compare, NOT `created` timestamp — fixes the "-latest" alias timestamp-refresh mis-sort) are carved into the pure, dependency-free `ui/ModelVersionOrdering` (`versionKey` / `versionNums` / `compareByVersionAsc`); `loadModelList` keeps the catalog fetch + JSON building inline and delegates the ordering. Pinned by `ModelVersionOrderingTest`. **Attachment MIME fallback extracted (Phase 3 cut):** the extension→MIME `when` inside `inferMimeType` (the fallback used when `Files.probeContentType` returns null on macOS/JBR — a null would route an image as a plain file and miss the vision path) is carved into the pure, dependency-free `ui/AttachmentMimeTypes.fromExtension(ext)`; `inferMimeType` keeps the probe inline and delegates the fallback. Pinned by `AttachmentMimeTypesTest`. **Answer-payload enrichment extracted (Phase 3 cut):** `buildEnrichedAnswerPayload`'s two branches (SIMPLE → resolve selected option ids to labels joined, or the `[SKIPPED]` sentinel; WIZARD → enriched JSON keyed by question id, with `"skipped":true` vs `"selected":[{id,label}]` so the LLM distinguishes "actively skipped" from "no options chosen") are carved into the pure `ui/AnswerPayloadEnricher.buildSimple(...)` / `buildWizard(...)` (operating on the public `Question` model); `buildEnrichedAnswerPayload` keeps only the `when(live.mode)` dispatch. Pinned by `AnswerPayloadEnricherTest`.
...
- **MessageStateHandler** (`session/MessageStateHandler.kt`, ~1080 lines) — Two-file JSON persistence (api_conversation_history.json + ui_messages.json). Atomic fsync'd writes via write-then-rename, per-session `kotlinx.coroutines.sync.Mutex` + inner `stateLock` for lock-free snapshot getters; throttled streaming saves with boundary flush (see Save cadence below).
...
- **Completion gates** (`loop/completion/`) — after `attempt_completion`, an ordered `CompletionGateChain` (pure, dependency-free state machine) can defer the loop's exit. Each `CompletionGate` is armed in turn: the loop injects its `nudge()` and continues; the gate clears either when a satisfying tool fires (`isSatisfiedByTool`) or when the agent re-issues `attempt_completion` (the universal advance signal). `AgentLoop` builds the chain pre-filtered and in order: **`MemoryReviewGate`** (gated by `AgentSettings.proactiveMemoryUpdatesEnabled`) then **`FeedbackGate`** (gated by `AgentSettings.agentFeedbackEnabled`). Empty chain → exit on first `attempt_completion`. Orchestrator-only (sub-agents complete via `task_report`); no LLM tool registered, so no schema-token cost. Replaced the old inline `awaitingFeedback`/`pendingCompletion` flags. Add a future post-completion step by implementing `CompletionGate` + adding it to the `buildList` in `AgentLoop` + a settings flag. `proactiveMemoryUpdatesEnabled` UI: Settings → Tools → Workflow Orchestrator → AI Agent → Advanced → "Memory". Pinned by `CompletionGateChainTest`, `CompletionGatesTest`, `AgentLoopCompletionGateTest`.
- **StreamBatcher** (`ui/StreamBatcher.kt`) — 16ms Swing timer coalesces rapid SSE chunks into single JCEF bridge calls (~5000 → ~300 per response). The timer starts via lock-free CAS on `append` (no per-chunk EDT post) and self-stops when a tick finds the buffer empty (P2-1/B12, Wave 1 perf). Sibling `PerToolStreamBatcher` follows the same lifecycle. **P1-12 (Wave 4):** ALL streaming paths now batch — main-agent thinking deltas through a dedicated `thinkingStreamBatcher` in `AgentController` (EDT-inline invoker; the block close runs `flush()` + `endThinking()` inside ONE EDT runnable so the final drain and the close are EDT-serial — a flush-then-post pair leaves a drained-but-undelivered window); sub-agent thinking deltas through an agentId-keyed `PerToolStreamBatcher` (`subAgentThinkingBatcher`, EDT-inline invoker, flushed before `endSubAgentThinking` and before the completion card so the CONTROLLER never reorders a tail delta behind its terminal event — a pre-existing producer-side `scope.launch` race at `SubagentRunner.kt` ~:369 can still land a delta after the terminal update; the flush bounds the loss to that producer window). Sub-agent PROSE was already batched at the source (`SubagentRunner.kt` `textBatcher`, flushed before the terminal progress event) — don't re-batch it controller-side. Both controller batchers are cleared in `clearStream()` (cancel/new chat) so a cancelled run's tail deltas can't deliver into the next session.
...
| **bamboo_builds** | 11 | build_status, trigger_build, get_build_log, get_test_results, etc. `build_status`, `recent_builds`, and `get_running_builds` resolve `branch → chainKey` (branch builds live under the branch plan key) and query that key only — no master-plan-key fallback (ChainKeyResolver's "no fallback to master" contract). `build_status`/`recent_builds` are running-aware (composite `getLatestBuild`+`getRunningBuilds`) and surface a non-silent warning when the running-check errors. |
...
**Hook-exemption:** `task_create`, `task_update`, `task_list`, `task_get` bypass `PreToolUse` and `PostToolUse` hooks entirely — they are internal bookkeeping and should never trigger user approval gates. They self-declare `isHookExempt = true` (replaces the old `AgentLoop.HOOK_EXEMPT` name set).
...
- **`PollingSource<T>`** (Phase 2 base, `monitor/PollingSource.kt`) — reusable base that wraps a `:core` `SmartPoller` and diffs successive snapshots. Subclasses implement `suspend fun fetch(): T?` (one async read; null = no-change so SmartPoller backs off) and a pure `fun diff(prev: T?, cur: T): List<MonitorEvent>`. The poll cycle is extracted into `internal suspend fun pollOnce(emit)` so it is unit-testable without a live SmartPoller.
- **`EventBusSource`** (Phase 2 base, `monitor/EventBusSource.kt`) — reusable base that subscribes to the `:core` `EventBus.events` `SharedFlow`, filters/maps `WorkflowEvent`s to `MonitorEvent`s via a pure `fun map(event): MonitorEvent?`, and performs **mandatory start-hydration** via `suspend fun hydrate(): MonitorEvent?` (EventBus is replay=0, so any event firing before subscription is lost — `hydrate()` seeds current state once on start). No domain consumer yet in Phase 2; first used by PR-state in Phase 3.
- **`BambooDiff`** (Phase 2, `monitor/BambooDiff.kt`) — pure state-transition logic for the Bamboo monitor (zero IDE deps). `enum Level { BUILD, STAGE, JOB }`; `diff(monitorId, level, stageName, jobName, prev, cur)`. Terminal `Failed` → `ALERT`, any other transition (incl. `Successful`) → `NOTABLE`. First poll (`prev==null`) emits only when the watched build/stage/job is already in a terminal state. A `buildNumber` change between snapshots (consecutive same-result builds) is also treated as a notable transition so back-to-back runs both fire.
- **`BambooMonitorSource`** (Phase 2, `monitor/BambooMonitorSource.kt`) — `PollingSource<BuildResultData>` over `BambooService`. Resolves `planKey`+`branch`→`chainKey` lazily: `branch==null` → `planKey`; else `getPlanBranches(planKey)` → first branch whose `name`/`shortName` equals `branch` (case-insensitive) → `.key`. `fetch()` is a composite: first calls `getRunningBuilds(chainKey)` and, on error or empty result, retries with the master `plan_key` (the branch-collection endpoint may 404 while `/latest` works); prefers the highest-`buildNumber` live build; falls back to `getLatestBuild(chainKey)` only when no live build exists (null on `isError`). This lets the monitor track queued→running→finished transitions. `diff` delegates to `BambooDiff`.
...
Deferred, registered under category `"Utilities"` in `AgentService.registerAllTools()`. Actions: `start | list | stop | status`. Sources: `source=shell`, (Phase 2) `source=bamboo`, (Phase 3) `source=pull_request`, (Phase 4) `source=jira_ticket` / `source=jira_sprint`, and (Phase 5) `source=sonar_gate` / `source=sonar_issues`. The `start` branch is a `when(source)` factory dispatching to `startShell` / `startBamboo` / `startPullRequest` / `startJiraTicket` / `startJiraSprint` / `startSonarGate` / `startSonarIssues`. Shell parameters: `command` (shell command to run), `filter` (regex — **case-sensitive by default; `(?i)` prefix for case-insensitive**). Bamboo parameters: `plan_key` (required), `branch` (omit for main plan), `level` (`build`|`stage`|`job`, default `build`), `stage_name` (required for stage/job), `job_name` (required for job). Pull-request parameters: `pr_id` (numeric, required), `aspects` (comma-separated subset of `state,reviews,comments`; default all three), `repo_name` (optional slug override). Jira-ticket parameters: `ticket_key` (e.g. `PROJ-123`, required). Jira-sprint parameters: `board_id` (numeric, resolves active sprint via `getAvailableSprints`) and/or `sprint_id` (numeric, polls directly; at least one required). Sonar-gate parameters: `project_key` (required), `branch` (optional). Sonar-issues parameters: `project_key` (required), `branch` (optional), `min_severity` (`INFO`|`MINOR`|`MAJOR`|`CRITICAL`|`BLOCKER`; default: all severities (no filter) when omitted). Common: `description` (label), `monitor_id` (for stop/status). `allowedWorkers = {ORCHESTRATOR, CODER}`. `validateStart()` (shell), `validateBambooStart(planKey, level, stageName, jobName)` (bamboo), `validatePullRequestStart(prIdRaw, aspects)` (pull_request), `validateJiraTicketStart(ticketKey)` (jira_ticket), `validateJiraSprintStart(boardIdRaw, sprintIdRaw)` (jira_sprint), `validateSonarGateStart(projectKey)` (sonar_gate), and `validateSonarIssuesStart(projectKey, minSeverity)` (sonar_issues) are pure companion methods (unit-testable), dispatched by source in `execute`. `renderStatus(handle)` is a pure companion method that formats one monitor's state + buffered matched-event lines (used by `action=status`). On `start`, a UUID-prefixed id is generated and returned (`shell-{8char}` / `bamboo-{8char}` / `pr-{8char}` / `jira-ticket-{8char}` / `jira-sprint-{8char}` / `sonar-gate-{8char}` / `sonar-issues-{8char}`). `BambooService` / `BitbucketService` / `JiraService` / `SonarService` are resolved via `ServiceLookup` (injected as optional provider lambdas for testability); unconfigured services return graceful errors. A shell monitor auto-removes on process exit, so `stop`/`status` on an already-exited shell monitor returns `No monitor with id …` (expected); Bamboo, PR, Jira, and Sonar sources run until `monitor stop` / session dispose.

**`source=pull_request` (Phase 3):** Composed by `PullRequestMonitorSource` which wires up to three child sources under ONE monitor id (all children stamp the parent id so the per-monitor wake-budget/flood ledger stays 1:1):
- `aspect=state` — `PrStateSource` (`EventBusSource`) subscribes to `WorkflowEvent.PullRequestMerged`, `PullRequestDeclined`, `PullRequestApproved`; performs start-hydration via `bitbucket.getPullRequestDetail()` so a PR that was already merged/declined before subscription is not silently missed.
- `aspect=reviews` — `PrReviewsSource` (`PollingSource`) polls `bitbucket.getPullRequestParticipants()` and diffs participant approval-status between snapshots.
- `aspect=comments` — two sources: `PrBlockerCountSource` (`PollingSource`) polls `bitbucket.getBlockerCommentsCount()`; `PrCommentsTotalSource` (`EventBusSource`) subscribes to `WorkflowEvent.PrCommentsUpdated`.

`validatePullRequestStart` is intentionally STRICTER than `PullRequestMonitorSource.parseAspects`: unknown aspect tokens are rejected at the tool boundary (fail-closed for the LLM), whereas `parseAspects` silently ignores them (post-validation default path).

**`source=jira_ticket` (Phase 4):** `JiraTicketMonitorSource` is a `PollingSource<JiraIssue>` that polls `JiraService.getTicket(ticketKey)`. `diff` fires a `NOTABLE` event on any status or assignee change. Requires `JiraService` resolved via `ServiceLookup.jira` (injected as `jiraProvider` lambda for testability); no `:core` changes were needed.

**`source=jira_sprint` (Phase 4):** `JiraSprintMonitorSource` is a `PollingSource<List<JiraIssue>>` that resolves the active sprint lazily: when `sprintId` is supplied, the sprint is polled directly; when only `boardId` is given, the first active sprint is resolved once via `JiraService.getAvailableSprints(boardId)` and cached. `fetch()` polls `JiraService.getSprintIssues(resolvedSprintId)`. `diff` emits a `NOTABLE` event for each issue whose status changes between snapshots, and for issues added to or removed from the sprint membership. Requires `JiraService` resolved via `ServiceLookup.jira` (injected as `jiraProvider` lambda for testability); no `:core` changes were needed (active-sprint resolution happens in-source; no `getActiveSprint` helper or new event was added).

**`source=sonar_gate` (Phase 5):** `SonarGateSource` is an `EventBusSource` that subscribes to `project.service<EventBus>().events` and maps `WorkflowEvent.QualityGateResult(projectKey, passed)` to a `MonitorEvent`: `passed=true` → `NOTABLE`, `passed=false` → `ALERT`. Start-hydration calls `SonarService.getQualityGateStatus(projectKey, branch)` — status `OK` → `NOTABLE`, status `ERROR` → `ALERT`, any other status (e.g. `WARN`) or `isError=true` → no hydration event. `SonarService` is resolved via `ServiceLookup.sonar` (injected as `sonarProvider` lambda for testability).

**`source=sonar_issues` (Phase 5):** `SonarIssuesSource` is a `PollingSource<List<SonarIssue>>` that polls `SonarService.getIssues(projectKey, branch)` and applies a client-side `IssueSeverity.meetsMinSeverity(minSeverity)` filter before diffing. Diff is via the shared `ListDiff.byKey` helper (see below): new issue with severity `BLOCKER` or `CRITICAL` → `ALERT`; new issue with lower severity → `NOTABLE`; resolved issue → one `NOTABLE` event with count; retained issues whose status changes are **not** emitted in v1. `SonarService` is resolved via `ServiceLookup.sonar` (injected as `sonarProvider` lambda for testability).

**`ListDiff` shared helper** (`monitor/ListDiff.kt`): pure, zero-dep utility. `ListDiff.byKey(previous, current, keyOf): Changes(added, removed, retained)`. Used by both `JiraSprintDiff` and `SonarIssuesDiff` to compare list snapshots by key without ordering assumptions.
...
- **(7) Monitor lifecycle params (terminal-state auto-stop SHIPPED 2026-06-10, P1-8).** The design-spec §5 `lifecycle: until_exit | persistent | timeout` + `timeout_ms` params are still NOT implemented — every monitor runs `persistent`-style until `monitor stop` / session dispose. **Terminal-state auto-stop is now built into `PollingSource`**: subclasses may override `isTerminal(current): Boolean` (default false); when `pollOnce` observes a non-terminal → terminal TRANSITION it emits a final NOTABLE `"monitor auto-stopped: watched resource reached terminal state"` event and calls `stop()` on its own poller. A FIRST poll that is already terminal does NOT auto-stop (a monitor on a plan whose last build finished long ago keeps waiting for the next build). Implemented by `BambooMonitorSource` (overall build state Successful/Failed via `BambooDiff.isTerminalState` — the same terminal notion the diff uses; applies to BUILD/STAGE/JOB levels since a finished build's stages/jobs can't transition further). Other polling sources intentionally do NOT override it — their snapshots can't carry terminality (`PrReviewsSource` = participant list, `PrBlockerCountSource` = count, Jira ticket/sprint = reopenable, Sonar issues = re-analyzable); `PrStateSource` is an `EventBusSource` (no poll loop to stop). After the self-stop, the registration-time `onSelfStop` callback (wired in `MonitorTool.startBamboo` and `AgentMonitorCoordinator.reArmMonitors`, mirroring the shell path's `onExit`) marks the pool handle EXITED via `MonitorPool.markExited(sessionId, id, null)` — `monitor status`/`list` truthfully show EXITED and the 5-per-session RUNNING-cap slot is freed; `monitor stop <id>` still resolves while the exited handle is retained. The monitor's `MonitorSpec` persistence is deliberately KEPT (unlike the shell path): a session resume re-arms the watch with a fresh snapshot, and the first-poll rule means the re-armed monitor waits for the NEXT build instead of insta-stopping on the already-terminal one. Pinned by `PollingSourceTerminalAutoStopTest` (incl. the pool-EXITED + wiring pins) + the terminal cases in `BambooMonitorSourceTest`.
...
- **Guards:** sub-agent exclusion is the **name filter** in `SpawnAgentTool.resolveConfigToolsTiered`
  ...
  (they never reach `onComplete`, so auto-finish couldn't hold). **Not** in the mutating set (does not declare `isMutating=true`) →
...
- **Unified Tool Stop** (`tools/cancel/`) — per-tool-call Stop that aborts only that tool, feeds `"[Stopped by user]"` back to the LLM, and lets the agent loop **continue**.
  - **`ToolCancellationRegistry`** — global `ConcurrentHashMap<toolCallId, Job>`; `register`/`unregister`/`cancel(id):Boolean` (cancels with `UserStopCancellationException`). Thread-safe: `cancel()` fires from the EDT/JCEF bridge while `register`/`unregister` run on `Dispatchers.IO`.
  - **`ToolStopCoordinator.requestStop(toolCallId)`** — two-layer precedence: `ProcessRegistry.kill` first (process tools keep their hard-kill + partial output); else `ToolCancellationRegistry.cancel` (cooperative coroutine cancel). Wired from `AgentController.setCefKillCallback` (the existing `_killToolCall` JCEF bridge).
  - **The funnel (`AgentLoop.executeToolCalls()`)** — wraps each `tool.execute` in `coroutineScope { register(toolCallId, callJob); try { ... } finally { unregister } }`. On a `CancellationException` whose cause chain contains `UserStopCancellationException` (`isUserStop`), calls `ensureActive()` then returns `stoppedByUserResult(toolName)` (`isError=false`) so the loop **continues**. A genuine loop-cancel re-throws. `withTimeoutOrNull` stays INSIDE the scope so a tool timeout is not misread as a user stop.
  - **Webview** — universal Stop button in `ToolCallChain.tsx` when `status === 'RUNNING'`, suppressed for `STOP_SUPPRESSED_TOOLS = {run_command, background_process, ask_user_input, agent}`. `run_command` has an in-terminal Stop; `background_process` has its own `kill` action; `ask_user_input` has its own answer affordance; `agent` uses the per-worker `SubAgentView` Kill → `cancelAgent` → `runner.abort()`.
  - **Sub-agents** — stopped via the pre-existing `agentId`-keyed per-worker Kill (`abort()` sets `abortRequested` + `brain.cancelActiveRequest()`). `SubagentRunner` re-throws `CancellationException` instead of swallowing it when `abortRequested` is `false` (structured-concurrency teardown).
  - **Blocking-tool cancellation (Phase 3)** — `find_references`/`find_definition` use `smartReadAction(project)` (a coroutine-cancellation-aware suspend read action); `run_inspections`/`diagnostics` add per-element `ProgressManager.checkCanceled()` in their PSI visitor walks (including `JavaKotlinProvider`/`PythonProvider.getDiagnostics`); `db_query` registers `invokeOnCompletion { stmt.cancel() }` to cancel the JDBC statement.
  - **Known limitation** — ~12 other PSI tools (`find_implementations`, `call_hierarchy`, `type_hierarchy`, `file_structure`, `list_quickfixes`, `problem_view`, etc.) use `ReadAction.nonBlocking().executeSynchronously()` — the universal Stop button appears on them but only abandons the result (work runs to completion); migrating them is future work.
...
- **ApprovalPolicy**: Per-tool approval rules, consumed via `ApprovalPolicy.forTool(tool: AgentTool)`. Three effective policies: `ALWAYS_APPROVE` (no gate), `ALLOW_FOR_SESSION` (user can grant once-per-session), `ALWAYS_PER_INVOCATION` (must approve every time). Tool declares `requiresApproval` + `allowSessionApproval` (if false → per-invocation). `run_command` declares `allowSessionApproval=false` — it can never be allowed for session.
...
**Save cadence (Wave 2 perf, 2026-06-10):** Mutex-guarded with throttled-plus-boundary-flush semantics — memory is authoritative, disk lags only mid-burst. PARTIAL streaming updates (`updateLastPartialMessage`, the per-SSE-chunk path) write `ui_messages.json` at most once per 500ms (`UI_SAVE_THROTTLE_MS`); `addToApiConversationHistory` coalesces within 1s bursts (`API_SAVE_THROTTLE_MS`). Flush boundaries: every non-partial mutation, every bulk rewrite (overwrite/truncate/collapse/prune/redact), `saveBoth()` (abortStream + makeFailed terminal flush), and stream finalize (partial→false). Crash window ≤1s of trailing appends; the resume path tolerates a trailing truncated exchange, and every write is a full atomic snapshot so disk is always a consistent prefix of memory. Clock seam `uiSaveClock` (constructor param) drives all three throttles for deterministic tests. `sessions.json` totals are incremental counters (recomputed on bulk rewrites), flushed at turn boundaries (B15).
...
### Capture trigger
`AgentLoop` calls `checkpointStore.captureIfFirstTouch(currentUserMsgTs, path)` for every path argument of every mutating tool invocation (`isMutating=true`), just before the tool executes. First touch per (msg, path) copies bytes; subsequent calls no-op. Non-existent paths are recorded in `meta.json.createdPaths` instead.
...
- **Editor-tab mirrors are separate Chromium instances by design (accepted P2-3):** each "View in Editor" tab hosts its own `JBCefBrowser` at ~150-250 MB per browser — JCEF can't share one render surface across the tool window and editor tabs, and the per-tab cost is accepted. Mirrors receive output via `AgentDashboardPanel.addMirror` broadcast plus a bounded `ReplayRing` replay log (P2-2/B7: O(1) add, oldest entry evicted on overflow so a late-joining mirror replays the NEWEST content; capped at 5000 entries). Recording is UNCONDITIONAL — `replayStateTo` is the only content source for a late-opened mirror, so do NOT gate recording on mirror presence (tried in W4-B3, reverted in review: it left the first late-opened editor tab and any close-then-reopen blank).
- **Tool-window visibility signal (P2-11):** `document.hidden` never flips inside embedded JCEF (the page stays "visible" to Chromium when the tool window is hidden), so `AgentController.subscribeToToolWindowVisibility()` listens on `ToolWindowManagerListener.TOPIC` and pushes the real visibility of the "Workflow" tool window into the page on each show/hide transition. Contract: `window.__wfToolWindowVisible: boolean` (`undefined` until the first transition — consumers must treat `undefined` as visible) + a `wf-visibility-change` `CustomEvent` on `window` with `detail: { visible: boolean }`. Kotlin-side signal only for now; webview 1s tickers (UsageIndicator etc.) can adopt it later. Only the primary tool-window browser receives the signal — editor-tab mirrors are independent, always-visible browsers.
...
1. **ThinkingTagSplitter** (Kotlin, `agent/ui/ThinkingTagSplitter.kt`): pure state machine that pulls `<thinking>...</thinking>` blocks out of the assistant text. Holds back potential tag-prefix bytes across chunk boundaries (`<thi` arriving alone doesn't leak as text). Three Part variants: `Text` for prose, `ThinkingDelta(text)` for incremental thinking bytes (emitted live as each chunk arrives — same cadence as prose, NOT buffered to the close tag), and `ThinkingEnd` to mark the close of a non-empty block. Text parts continue through `streamBatcher`; `ThinkingDelta` routes through the dedicated 16ms `thinkingStreamBatcher` (P1-12 — thinking models emit thousands of chunks; pre-fix each chunk was one `executeJavaScript`) → `dashboard.appendToThinking()` → JS `appendToThinking` → `chatStore.appendToThinking()` → live render in `ChatFooter` as `<ThinkingView isStreaming={true}>` (prompt-kit `Reasoning` + `TextShimmer`, no auto-collapse). `ThinkingEnd` posts ONE `invokeLater` runnable that runs `thinkingStreamBatcher.flush()` (EDT-inline delivery) then `dashboard.endThinking()` — final drain and block close execute in the same EDT event, so the tail delta cannot be overtaken (a flush-then-post pair left a window where a timer tick had drained but not yet delivered) → JS `endThinking` → `chatStore.endThinking()` which flushes the accumulated buffer into `messages` as a `say: 'REASONING'` UiMessage rendered by `ChatView.renderItem` as `<ThinkingView isStreaming={false}>` (auto-collapses after 600 ms, user can re-expand). Lockstep flush/reset with `streamBatcher` via `AgentController.flushStream()` / `clearStream()`. Fallback (non-JCEF) panel still receives the full block one-shot via `RichStreamingPanel.appendThinking()` — `AgentDashboardPanel` accumulates deltas in `thinkingFallbackBuffer` and calls fallback only on `endThinking()` when `cefPanel == null`.
2. **StreamBatcher** (Kotlin, `agent/ui/StreamBatcher.kt`): 16ms Swing timer coalesces rapid chunks into single bridge calls (~5000 → ~300 per response); CAS-started on append, self-stops on an empty tick (P2-1/B12).
