...
Deferred, registered under category `"Utilities"` in `AgentService.registerAllTools()`. Actions: `start | list | stop | status`. Sources: `source=shell` and (Phase 2) `source=bamboo`. The `start` branch is a `when(source)` factory dispatching to `startShell` / `startBamboo`. Shell parameters: `command` (shell command to run), `filter` (regex — **case-sensitive by default; `(?i)` prefix for case-insensitive**). Bamboo parameters: `plan_key` (required), `branch` (omit for main plan), `level` (`build`|`stage`|`job`, default `build`), `stage_name` (required for stage/job), `job_name` (required for job). Common: `description` (label), `monitor_id` (for stop/status). `allowedWorkers = {ORCHESTRATOR, CODER}`. `validateStart()` (shell) and `validateBambooStart(planKey, level, stageName, jobName)` (bamboo) are pure companion methods (unit-testable), dispatched by source in `execute`. `renderStatus(handle)` is a pure companion method that formats one monitor's state + buffered matched-event lines (used by `action=status`). On `start`, a UUID-prefixed id is generated and returned (`shell-{8char}` / `bamboo-{8char}`). `BambooService` is resolved via `ServiceLookup.bamboo(project)` (injected as an optional `bambooProvider` ctor lambda for testability); an unconfigured Bamboo returns a graceful `Bamboo is not configured.` error. A shell monitor auto-removes on process exit, so `stop`/`status` on an already-exited shell monitor returns `No monitor with id …` (expected); Bamboo sources have no process exit and run until `monitor stop` / session dispose.
...
`MonitorEventTest`, `MonitorSourceContractTest`, `MonitorManagerTest`, `MonitorWakeRoutingTest`, `ShellCommandSourceTest`, `MonitorHandleTest`, `MonitorSettingsDefaultsTest`, `MonitorToolTest`, `EnvironmentDetailsMonitorSectionTest`, `PollingSourceTest`, `EventBusSourceTest`, `BambooDiffTest`, `BambooMonitorSourceTest`.
