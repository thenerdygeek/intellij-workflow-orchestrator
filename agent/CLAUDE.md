...
Deferred, registered under category `"Utilities"` in `AgentService.registerAllTools()`. Actions: `start | list | stop | status`. Sources: `source=shell`, (Phase 2) `source=bamboo`, and (Phase 3) `source=pull_request`. The `start` branch is a `when(source)` factory dispatching to `startShell` / `startBamboo` / `startPullRequest`. Shell parameters: `command` (shell command to run), `filter` (regex — **case-sensitive by default; `(?i)` prefix for case-insensitive**). Bamboo parameters: `plan_key` (required), `branch` (omit for main plan), `level` (`build`|`stage`|`job`, default `build`), `stage_name` (required for stage/job), `job_name` (required for job). Pull-request parameters: `pr_id` (numeric, required), `aspects` (comma-separated subset of `state,reviews,comments`; default all three), `repo_name` (optional slug override). Common: `description` (label), `monitor_id` (for stop/status). `allowedWorkers = {ORCHESTRATOR, CODER}`. `validateStart()` (shell), `validateBambooStart(planKey, level, stageName, jobName)` (bamboo), and `validatePullRequestStart(prIdRaw, aspects)` (pull_request) are pure companion methods (unit-testable), dispatched by source in `execute`. `renderStatus(handle)` is a pure companion method that formats one monitor's state + buffered matched-event lines (used by `action=status`). On `start`, a UUID-prefixed id is generated and returned (`shell-{8char}` / `bamboo-{8char}` / `pr-{8char}`). `BambooService` / `BitbucketService` are resolved via `ServiceLookup` (injected as optional provider lambdas for testability); unconfigured services return graceful errors. A shell monitor auto-removes on process exit, so `stop`/`status` on an already-exited shell monitor returns `No monitor with id …` (expected); Bamboo and PR sources run until `monitor stop` / session dispose.
