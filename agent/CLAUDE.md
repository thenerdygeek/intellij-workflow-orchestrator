...
| **bamboo_builds** | 11 | build_status, trigger_build, get_build_log, get_test_results, etc. `build_status` and `recent_builds` are running-aware: composite `getLatestBuild`+`getRunningBuilds` with master-plan-key fallback; surface a non-silent warning when the running-check fails on all attempts. |
