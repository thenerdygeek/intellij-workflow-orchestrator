...
class ContextManagerTest {
    ...
    // ---- Task progress (ported from Cline's FocusChain) ----

    @Nested
    inner class TaskProgressTracking {

        @Test
        fun `setTaskProgress stores and returns parsed progress`() {
            val md = "- [x] Step one\n- [ ] Step two"
            val progress = cm.setTaskProgress(md)
        ...
        @Test
        fun `getTaskProgress returns stored markdown`() {
            cm.setTaskProgress("- [x] Done\n- [ ] Pending")

            val stored = cm.getTaskProgress()
            assertEquals("- [x] Done\n- [ ] Pending", stored)
        }

        @Test
        fun `getTaskProgress returns null before any progress is set`() {
            assertNull(cm.getTaskProgress())
        }

        @Test
        fun `getTaskProgressParsed returns parsed object`() {
            cm.setTaskProgress("- [x] A\n- [ ] B\n- [x] C")

            val parsed = cm.getTaskProgressParsed()
            assertNotNull(parsed)
            assertEquals(3, parsed!!.totalCount)
            assertEquals(2, parsed.completedCount)
        }

        @Test
        fun `setTaskProgress trims whitespace`() {
            cm.setTaskProgress("  \n  - [x] Done  \n  ")

            val stored = cm.getTaskProgress()
            assertEquals("- [x] Done", stored)
        }

        @Test
        fun `setTaskProgress returns null for blank input`() {
            val result = cm.setTaskProgress("   ")
            assertNull(result)
            assertNull(cm.getTaskProgress())
        }

        @Test
        fun `task progress survives compaction`() = runTest {
            val fakeBrain = FakeLlmBrain(summaryResponse = "TASK: test\nFILES: none\nDONE: none")
            cm = ContextManager(maxInputTokens = 200)
            cm.setSystemPrompt("sys")

            // Set task progress
            cm.setTaskProgress("- [x] Read the file\n- [ ] Fix the bug")
            ...
            // Task progress should survive compaction (it's stored separately)
            val progressAfter = cm.getTaskProgress()
            assertEquals("- [x] Read the file\n- [ ] Fix the bug", progressAfter)
        }

        @Test
        fun `task progress updates replace previous value`() {
            cm.setTaskProgress("- [ ] Step one\n- [ ] Step two")
            cm.setTaskProgress("- [x] Step one\n- [ ] Step two")

            val stored = cm.getTaskProgress()
            assertEquals("- [x] Step one\n- [ ] Step two", stored)

            val parsed = cm.getTaskProgressParsed()
            assertTrue(parsed!!.items[0].completed)
        }
    }
