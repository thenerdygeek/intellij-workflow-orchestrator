...
class ModelFallbackLoopTest {
    ...
    @Test
    fun `onApiCallStart receives current model ID`() = runTest {
        val modelChain = listOf("model-primary", "model-fallback1", "model-fallback2")
        val fallbackManager = ModelFallbackManager(modelChain)

        // Primary brain: first call errors, no second call expected from primary
        val primaryBrain = SequenceBrain(
            modelId = "model-primary",
            responses = listOf(
                ApiResult.Error(ErrorType.NETWORK_ERROR, "Timeout")
            )
        )

        // Fallback brain: succeeds with completion
        val fallbackBrain = SequenceBrain(
            modelId = "model-fallback1",
            responses = listOf(
                ApiResult.Success(completionToolCallResponse())
        ...
        val apiCallModels = mutableListOf<String>()
        ...
        val loop = AgentLoop(
            ...
            fallbackManager = fallbackManager,
            brainFactory = brainFactory,
            onApiCallStart = { modelId -> apiCallModels.add(modelId) }
        )

        val result = loop.run("Fix the bug")

        assertTrue(result is LoopResult.Completed, "Expected Completed but got $result")
        // First call was on primary, second on fallback
        assertTrue(apiCallModels.size >= 2, "Expected at least 2 API calls, got ${apiCallModels.size}")
        assertEquals("model-primary", apiCallModels[0])
        assertEquals("model-fallback1", apiCallModels[1])
    }
}
