...
class DropFrameToolTest {
    private val controller = mockk<AgentDebugController>(relaxed = true)
    private val project = mockk<Project>(relaxed = true)
    private val tool = DropFrameTool(controller)

    @Test
    fun `metadata is correct`() {
        assertEquals("drop_frame", tool.name)
        assertTrue(tool.description.contains("Rewind execution"))
        assertTrue(tool.description.contains("Variable state is NOT reset"))
    }

    @Test
    fun `session_id and frame_index are optional`() {
        assertTrue(tool.parameters.required.isEmpty())
        assertTrue(tool.parameters.properties.containsKey("session_id"))
        assertTrue(tool.parameters.properties.containsKey("frame_index"))
    }

    @Test
    fun `frame_index parameter is integer type`() {
        assertEquals("integer", tool.parameters.properties["frame_index"]?.type)
    }
    ...
    @Test
    fun `returns error for negative frame_index`() = runTest {
        val result = tool.execute(buildJsonObject {
            put("frame_index", -1)
        }, project)
        assertTrue(result.isError)
    ...
    fun `produces valid tool definition`() {
        ...
        assertEquals("drop_frame", def.function.name)
