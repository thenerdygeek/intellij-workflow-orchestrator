...
class MavenEffectivePomTool : AgentTool {
    ...
    override val description = "Show Maven plugin configurations from the effective POM. Reveals: compiler source/target, surefire/failsafe includes/excludes, spring-boot-maven-plugin settings."
    override val parameters = FunctionParameters(
        properties = mapOf(
            ...
            "plugin" to ParameterProperty(
                type = "string",
                description = "Optional: filter by plugin artifactId (e.g. 'maven-compiler-plugin', 'spring-boot-maven-plugin')."
            )
        ),
        required = emptyList()
    )
