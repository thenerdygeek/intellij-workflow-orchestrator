...
class CreateFileTool : AgentTool {
    ...
    override fun documentation(): ToolDocumentation = toolDoc("create_file") {
        ...
        downside("Plan mode blocks this tool entirely (it declares `isMutating = true`). Even an LLM that hallucinates a create_file call in plan mode will be rejected at the AgentLoop execution guard.")
        downside("Approval is required (`requiresApproval=true, allowSessionApproval=true` (session-approvable)) — the user can grant 'allow for session', but the first call always pauses the loop for human confirmation. In headless / CI-style usage this is a friction point.")
