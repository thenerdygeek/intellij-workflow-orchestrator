...
class PlanModeRespondTool : AgentTool {
    ...
    override fun documentation(): ToolDocumentation = toolDoc("plan_mode_respond") {
        ...
        llmMistake("Tries to use edit_file or create_file to write the plan to disk while in plan mode — blocked twice (schema filter strips write tools; AgentLoop's plan-mode execution guard (per-tool `isMutating`) rejects cached calls). The LLM should put the plan in the `response` parameter, not try to land it as a file.")
