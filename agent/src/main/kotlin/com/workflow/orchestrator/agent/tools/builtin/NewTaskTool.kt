...
class NewTaskTool : AgentTool {
    ...
    override fun documentation(): ToolDocumentation = toolDoc("new_task") {
        summary {
            technical(
                "Handoff PROPOSAL (not auto-fire): returns ToolResultType.HandoffProposed(context). AgentLoop fires onHandoffProposed (which renders a preview card with two buttons) and SUSPENDS on userInputChannel. The user chooses: 'Start fresh session' sends HANDOFF_FORK_SENTINEL → the loop returns LoopResult.SessionHandoff → AgentController spawns a NEW session whose first user message is `Continue from the previous session. Here is the preserved context:\\n\\n<context>` and the old session is marked COMPLETED; 'Keep chatting here' sends HANDOFF_DECLINE_SENTINEL → the loop returns LoopResult.Completed and the current session continues unchanged (the proposed summary is discarded). The LLM is expected to author a 5-section structured summary (Current Work / Key Technical Concepts / Relevant Files / Problem Solving / Pending Tasks) so the fresh session has enough context to continue. Cline-faithful (`new_task.ts`, user-confirmed); orchestrator-only via WorkerType.ORCHESTRATOR."
            )
            plain(
        ...
        params {
            required("context", "string") {
                ...
                whenPresent(
                    "After validation (must be non-blank), the tool returns ToolResult.handoffProposed with the context as both the content and the typed payload. AgentLoop sees ToolResultType.HandoffProposed, renders the preview card, and suspends for the user's decision. On 'Start fresh session' it exits with LoopResult.SessionHandoff, the previous session is finalised as COMPLETED, ContextManager + SessionApprovalStore are cleared, and AgentService.startHandoffSession spawns a fresh session whose first user message is `Continue from the previous session. Here is the preserved context:\\n\\n<context>`. On 'Keep chatting here' the loop returns control to the user and the current session is untouched."
                )
