...
class ThinkTool : AgentTool {
    ...
    override fun documentation(): ToolDocumentation = toolDoc("think") {
        ...
        counterfactual(
            "Without `think`, the LLM has two worse options. (1) Emit a text-only assistant turn to reason out loud — but text-only turns trigger AgentLoop's escalating no-tool nudges (driven by the inline `maxConsecutiveMistakes = 3` budget in `AgentLoop.run`) which then either suspend on `userInputChannel` for the orchestrator or fail outright in a sub-agent, so reasoning gets punished. (2) Smuggle deliberation into the args of a real tool (e.g. a verbose `read_file` rationale, or a `<thinking>` block inside `attempt_completion`) — which couples reasoning to side effects and makes loop traces harder to read. Net cost of dropping it: lower-quality reasoning interleaved with real tool calls, plus the extra round-trips to satisfy the no-tool-nudge logic."
        )
