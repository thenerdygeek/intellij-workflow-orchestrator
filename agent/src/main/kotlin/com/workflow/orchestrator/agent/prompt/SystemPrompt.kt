 ...
 *     search_files -> search_files (same), list_files -> list_files (same),
 *     list_code_definition_names -> list_code_definitions (same concept),
...
You can track and communicate your progress on the overall task using a task progress checklist. Using task progress tracking ensures you remain on task and stay focused on completing the user's objective.

- When switching from PLAN MODE to ACT MODE, you must create a comprehensive todo list for the task.
- Todo list updates should be done silently -- do not announce these updates to the user.
...
- You can use the list_code_definitions tool to get an overview of source code definitions for all files at the top level of a specified directory. This can be particularly useful when you need to understand the broader context and relationships between certain parts of the code. You may need to call this tool multiple times to understand various parts of the codebase related to the task.
    - For example, when asked to make edits or improvements you might analyze the file structure in the initial environment_details to get an overview of the project, then use list_code_definitions to get further insight using source code definitions for files located in relevant directories, then read_file to examine the contents of relevant files, analyze the code and suggest improvements or make necessary edits, then use the edit_file tool to implement changes. If you refactored code that could affect other parts of the codebase, you could use search_files to ensure you update other files as needed.
