...
class SpringBeanGraphTool : AgentTool {
    override val name = "spring_bean_graph"
    override val description = "Show the dependency injection graph for a specific Spring bean: what it injects " +
        "(constructor params, @Autowired fields) and what other beans inject it."
    override val parameters = FunctionParameters(
        properties = mapOf(
            "bean_name" to ParameterProperty(
                type = "string",
                description = "Bean class name (simple or fully qualified). E.g., 'UserService' or 'com.example.UserService'."
            )
        ),
        required = listOf("bean_name")
    )
    ...
    override suspend fun execute(params: JsonObject, project: Project): ToolResult {
        ...
        val beanName = params["bean_name"]?.jsonPrimitive?.contentOrNull
            ?: return ToolResult(
                "Error: 'bean_name' parameter required",
                "Error: missing bean_name",
    ...
    private fun buildDependencyGraph(project: Project, beanName: String): String {
        ...
        // 1. What this bean injects (dependencies)
        ...
        // 2. What injects this bean (consumers)
    ...
    private fun findConsumers(project: Project, targetClass: PsiClass): List<ConsumerInfo> {
        ...
        // Search all Spring bean classes for constructor/field references to this type
