...
description: Live debugger control — breakpoints, stepping, variable inspection, runtime expression evaluation in a running JVM session. The ESCALATION path from systematic-debugging: load only once static analysis can't find the root cause and you have a specific runtime hypothesis. Triggers: 'step through', 'set a breakpoint', 'watch this variable', 'evaluate at runtime', thread deadlocks, race conditions. Do NOT use for initial investigation — start with systematic-debugging.
user-invocable: true
