...
description: Structured root-cause investigation — load BEFORE any fix attempt, even if the cause seems obvious. Triggers: bug, test failure, build failure, runtime error, exception, NPE/null pointer, crash, stack trace, CI failure, flaky test, 'not working', 'broken', 'wrong output', 'why does this'. Escalate to interactive-debugging only when static analysis can't find the cause.
user-invocable: true
