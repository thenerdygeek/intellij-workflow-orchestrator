...
3. If you want to pre-flight what's available (e.g. to tell the user which repos are options), use `delegation(action="list_targets")` — it **lists candidate target IDEs and their inbound readiness** (the same list the picker shows) without opening any UI. It does NOT list your active delegation handles (use `list_handles` for that). Status meanings:
   - `running` — IDE open AND inbound delegation is ON (accepts a delegation directly, no consent prompt).
...
- `DelegationRejected` / `DelegationDeclined` — the remote human declined consent. A **busy decline** is now self-describing: the reason leads with `ide_b_busy:` and NAMES the in-flight task the target tab is busy with (`ide_b_busy: agent tab is busy running session <sid> ('<title>')[, delegated by <repo> session <delegatorSessionId>]; user did not click Start within Ns`). If the in-flight task is itself delegated, the reason echoes ITS delegator session id — match that against your own `list_handles` `b_session_id` to recognize the blocker as YOUR OWN earlier task (and `wait` on it / let it finish instead of retrying blindly). You can retry once it's free.
- `DelegationExpired` — on a **continuation send** (`handle` present) this carries a specific reason so you know how to recover:
  ...
  - `ide_b_busy` — the target tab is busy with another task; the reason names the in-flight session (and, if it's delegated, its delegator session id — possibly YOUR own earlier task). `wait` on / let that finish, or retry shortly.
