...
- **Framework:** spring (actions: context, bean_graph, config, jpa_entities, etc.; also endpoints/boot_endpoints on IntelliJ Community), endpoints (actions: list, find_usages, export_openapi, export_http_scratch, list_async, service_graph — multi-framework on Ultimate/Pro/WebStorm), build (actions: maven_dependencies, gradle_dependencies, etc.)
