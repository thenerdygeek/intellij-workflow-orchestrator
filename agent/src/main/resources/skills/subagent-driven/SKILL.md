...
description: Execute an approved plan by dispatching parallel subagents (one per task) with spec + quality review gates. Load when a plan with 3+ tasks is approved (approve button, or 'execute the plan', 'start implementing', 'run the plan'). For 1-2 task plans, execute directly instead. Handles parallel dispatch, file-ownership coordination, and review gates.
user-invocable: true
