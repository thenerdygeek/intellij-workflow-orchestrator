...
User's idea
  ...
  → Ask clarifying questions (one at a time, use ask_questions for structured choices)
...
Ask questions **one at a time**. For questions with clear options, use `ask_questions` to render the interactive wizard with clickable choices. For open-ended questions, ask in chat.

### When to use `ask_questions` (wizard):
...
[{
  "id": "approach",
  "question": "How should the caching layer work?",
  "type": "single",
  "options": [
    {"id": "inmemory", "label": "In-Memory (ConcurrentHashMap)", "description": "Fast, simple, lost on restart"},
    {"id": "caffeine", "label": "Caffeine Cache", "description": "TTL, max-size, eviction policies"},
    {"id": "redis", "label": "External Redis", "description": "Shared across instances, persistent"},
    {"id": "other", "label": "Something else", "description": "I'll explain in chat"}
  ]
}]
```
...
[{
  ...
  "type": "single",
  "options": [
    {"id": "a", "label": "Approach A: [Name]", "description": "Recommended — [one-line reason]"},
    {"id": "b", "label": "Approach B: [Name]", "description": "[one-line trade-off]"},
    {"id": "c", "label": "Approach C: [Name]", "description": "[one-line trade-off]"},
    {"id": "discuss", "label": "Let's discuss more", "description": "I have questions or a different idea"}
  ]
}]
```
...
Once the user approves the design, use `create_plan` to generate an implementation plan with concrete steps. The plan card will render in the chat with approve/revise buttons.
...
| Write code during brainstorming | Premature implementation before design approval | Finish design, then create_plan, then implement |
