...
PR creation has moved to the `:pullrequest` module. `PrBar.openCreatePrDialog()` delegates via `CreatePrLauncher.getInstance()?.launch(project, scope, autoGenerateDescription = true)` (Build tab passes `true` to preserve auto-generate-on-open).
