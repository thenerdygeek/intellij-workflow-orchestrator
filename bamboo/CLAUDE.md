...
PR creation has moved to the `:pullrequest` module. `PrBar.openCreatePrDialog()` now delegates via `CreatePrLauncher.getInstance()?.launch(project, scope, autoGenerateDescription = true)` (Build tab passes `true` to preserve auto-generate-on-open). The legacy inline form submit path (`onSubmitPr`, `onRegenerateDescription` using `PrService.buildEnrichedDescription` PSI path) is unchanged.

`PrDescriptionGenerator` and `MarkdownToHtml` also moved to `:pullrequest/service/` — `:bamboo` no longer depends on `:pullrequest` (delegation happens via the `createPrLauncher` extension point in `:core`).
