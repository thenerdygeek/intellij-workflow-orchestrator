...
class PrBar(
     ...
     *
     * Phase 5: [CreatePrDialog] lives in :pullrequest; this call-site still performs
     * the prefetch inline and constructs the dialog directly via a temporary
     * `:bamboo → :pullrequest` compile dep. Phase 6 replaces both with a single
     * `CreatePrLauncher.getInstance()?.launch(...)` call.
     */
    private fun openCreatePrDialog() {
        ...
        scope.launch {
            ...
            // Resolve TicketContext once and hand it to the dialog as a pre-cached entry so
            // TicketChipInput marks the initial chip VALID immediately without a re-fetch.
            val ticketContext = if (ticketId.isNotBlank()) {
                jiraProvider?.getTicketContext(ticketId)
            } else null

            val initialTicketKeys: List<String> = if (ticketId.isNotBlank()) listOf(ticketId) else emptyList()
            val initialTicketContexts: Map<String, TicketContext> = if (ticketContext != null) {
                mapOf(ticketId to ticketContext)
            } else emptyMap()

            val defaultTitle = com.workflow.orchestrator.pullrequest.service.PrDescriptionGenerator
                .generateTitle(project, ticketContext, currentBranch)

            invokeLater {
                val dialog = com.workflow.orchestrator.pullrequest.ui.CreatePrDialog(
                    project = project,
                    scope = scope,
                    sourceBranch = currentBranch,
                    remoteBranches = remoteBranches,
                    initialTicketKeys = initialTicketKeys,
