...
class SourcegraphChatClient(
    ...
    /**
     * Delegates to [MessageSanitizer.sanitizeForAnthropic] — extracted to the shared
     * object so the `/stream` path in [BrainRouter] can apply the same Phase 1-5 rules.
     * Kept as a private helper to preserve all existing call-site names within this class.
     */
    private fun sanitizeMessages(messages: List<ChatMessage>): List<ChatMessage> =
        MessageSanitizer.sanitizeForAnthropic(messages)
