...
- `TagBuilderService` — builds `dockerTagsAsJson` payload (service-to-tag mappings) and detects the current build's docker tag. **Docker-tag detection is whole-log, per-job, and order-independent (2026-05-27):** the "Unique Docker Tag" marker is emitted near the END of ONE job's output (after the image is built/pushed), and Bamboo's REST returns a build's jobs in an *unstable* order (the probe returned the same plan's jobs in different orders across builds) — so the marker can be at the tail or middle of the concatenated log. `extractDockerTagFromLog` scans the WHOLE log (no head/tail window); `DOCKER_TAG_REGEX` is anchored + non-backtracking so `find()` is linear and short-circuits at the first match (runs once per terminal build / mount, not per poll tick). This replaces the old audit:F-5 head-window cap, whose "tag appears early" assumption was wrong and silently dropped end-of-log tags. The cache-cold REST fallback (`detectDockerTag`) scans EACH job's log (`BuildResultData.stages[].jobs[].resultKey`) rather than the chain/plan-level log (empty/404 on this Bamboo), and reports the job key that carried the tag. Pinned by `DockerTagRegexBoundTest` + `TagDetectionPerJobTest`.
...
- `AutomationPanel` — main panel with tag staging + queue + monitor sub-panels. Header has a **branch selector** (`branchCombo`, replaced the old passive PR-branch label 2026-05-25): on suite-select/tab-open it loads `BambooService.getPlanBranches(planKey)` (guarded by the `loadGeneration` stale-token), offers a `"default"` (= master, `branchKey=null`) entry plus each plan branch (`PlanBranchData.key`), and restores the persisted per-suite selection (falls back to default if the branch was deleted in Bamboo). The selected `branchKey` flows into `QueueEntry.branchKey`; `QueueService.doTrigger` triggers `branchKey ?: suitePlanKey`. The list is sorted alphabetically with `"default"` (master) pinned first. The combo is a `ComboBoxWithWidePopup`: the closed control stays capped at `ComboBoxWidth.DEFAULT` (via `bindBoundedWidth`, which also installs a full-name hover tooltip keyed on the branch label — set the renderer BEFORE `bindBoundedWidth` so the tooltip wrapper survives), while the expanded popup floors at `ComboBoxWidth.WIDE` (`setMinLength`) and grows to fit longer names. A `ComboboxSpeedSearch` (keyed on `BranchComboItem.label`) makes the popup type-to-filter. **Disabled** Bamboo branches render greyed (`ColoredListCellRenderer` + `GRAYED_ATTRIBUTES`, "(disabled)" suffix) and stay selectable; picking one pops a Yes/No dialog offering to enable it via `BambooService.enablePlanBranch` — on success the branch list re-renders enabled and stays selected, on cancel/failure the combo reverts to the committed `selectedBranchItem`. **The enable prompt fires only on a *deliberate commit* (dropdown close or focus-lost), NOT on the per-keystroke selection changes `ComboboxSpeedSearch` makes while type-filtering** — otherwise the modal popped mid-keystroke as soon as speed-search highlighted a disabled branch. The `branchCombo.addActionListener` only persists enabled/default branches (cheap idempotent commit); disabled branches are routed to `settleBranchSelectionFromUi()` via a `PopupMenuListener` (popup-close) + `FocusListener` (focus-lost), both deferred one EDT cycle. The settle decision is the pure, unit-tested `resolveBranchSettleAction(selected, committed, popupVisible, enableInFlight)` (`BranchSelectorLogic.kt`, pinned by `BranchSelectorSettleTest`): it suppresses the prompt while the popup is open, while an enable is already in flight (`branchEnableInFlight`), for enabled/default branches, and when the selection is unchanged. **Branch selection does NOT rescan the baseline** — the docker-tag baseline stays anchored to the master plan regardless of selected branch (`BaselineCacheService` is untouched, still keyed by `planKey`).
...
- `MonitorPanel` — list+detail of every queue entry, including terminal ones. Filter chips (All / Queued / Running / Failed / Completed) with `CANCELLED` bucketed under Failed. Sorted latest-first by `enqueuedAt`. Detail header shows **Cancel** for live entries and **Remove** for terminal entries (calls `QueueService.dismiss`). Exposes `onSelectionChanged: (RunEntry?) -> Unit`. **Perf (P1-21, 2026-06-10 audit):** per-entry poll updates are batched and applied in ONE EDT hop per cycle (`mergePolledEntries`), and `showRunDetail` skips teardown/rebuild when the next `RunEntry` is structurally equal to the rendered one.
