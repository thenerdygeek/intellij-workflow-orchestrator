...
| **T-Handover-b** | Delete dead accumulator paths in `HandoverStateService.handleEvent`; PR-scope remaining event handlers | `handover/service/HandoverStateService.kt`; existing tests | Existing handover tests pass; new: cross-branch event ignored | T-Handover-a |
| **T-Handover-c** | Verify `HandoverPlaceholderResolver` and `ShareTab` placeholders read from `WorkflowContext` (not stale `HandoverState` mirror); fix any drift | `handover/service/HandoverPlaceholderResolver.kt`; `handover/ui/tabs/ShareTab.kt` | Existing placeholder tests; possibly add focus-change → placeholder updates test | T-Handover-a |
