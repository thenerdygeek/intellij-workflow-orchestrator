...
| **R-SWAP-1** | `JiraApiClient.validateTicketKeys` (BranchChangeTicketDetector + TicketKeyCache) | `GET /rest/api/2/search?jql=key in (…)` chunked(100) | ✅ POST 200, 482ms | **SWAP** to `POST /rest/api/2/search` with body `{"jql":"key in (…)","fields":["summary","status"],"maxResults":N}`. Drop the `chunked(100)` loop. Single round-trip for any batch size. | S |
| **R-SWAP-2** | `MentionSearchProvider.searchTickets` (manual board+sprint walk) | various agile/board endpoints + filter | ✅ `GET /rest/api/2/issue/picker?query=…&showSubTasks=true` 200, 575ms | **SWAP** to issue-picker: returns history-grouped suggestions in one call. Drop the board enumeration. Keep the per-board cache only as a *prewarm* hint (still useful for offline boot). | M |
...
| **R-ADD-3** | New: changelog feed in `TicketDetailPanel` | `GET /rest/api/2/issue/{key}?fields=summary&expand=changelog` ✅ 200, 325ms | **ADD** | Render last N status/assignee/priority changes as "X changed status to Y · 3d ago" chips. Already part of the standard issue payload — add a section to the detail panel. | M |
