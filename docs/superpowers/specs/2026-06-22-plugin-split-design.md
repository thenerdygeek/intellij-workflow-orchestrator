...
**"NativeProtocol deletes the drift machinery" = 5-site conditional-bypass surgery**, not a no-op impl: `DialectDriftDetector` is wired into the persistence write path (`MessageStateHandler`), two prompt-assembly paths (`SystemPrompt`, `SubagentSystemPromptBuilder`), and resume redaction (`ResumeHelper`/`AgentService`). Native must bypass these via a flag the seam exposes — an explicit task. `MessageSanitizer` fuses transport + protocol concerns and `ContextManager`'s L4 compaction depends on its role-merge — so splitting it into the XML protocol is non-transport-local.
...
**Persisted-message-format migration (Phase 4).** `ApiMessage` stores an assistant turn as **one `ContentBlock.Text` with the tool call inline as XML** (the structured `ToolUse` variant is legacy/pre-2026-05-13); tool results persist as `"TOOL RESULT:"` text. Native `tool_use`/`tool_result` are a **different on-disk shape** → a **persisted-format migration + a per-message protocol discriminator** so resume can replay a session mixing XML and native turns. Unbudgeted before Round-2; now an explicit Phase-4 line item.
