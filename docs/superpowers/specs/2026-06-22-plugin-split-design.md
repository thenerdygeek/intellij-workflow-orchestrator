...
## 25. Phase 4a resolved (2026-06-30)

**Native Anthropic-direct LLM provider.** Plugin A can run the agent against `api.anthropic.com` with a user-supplied `x-api-key`, no Sourcegraph required (`AgentSettings.llmProvider == "anthropic"`). **Option A ("native wire, XML-internal"):** `AnthropicDirectBrain` (`:agent/ai/`) calls the Messages API natively (top-level `system` field, structured `tools:[]`, `tool_use`/`tool_result`, adaptive thinking + `output_config.effort` with `display:"summarized"`, proxy-aware OkHttp via the `:web` `IdeProxy`/`IdeTrust` triad) but **deterministically serializes the model's structured `tool_use` back into the canonical XML** the existing `AssistantMessageParser` + persistence already consume — so `ApiMessage` on-disk format, `ContextManager`, and the dialect-drift machinery are **unchanged** (persistence back-compat risk = zero). The `:core` native stack: `AnthropicModelCatalog` (+ `AnthropicModelCatalogService` adapter feeding `ContextManager` the real ~1M window instead of the 90K fallback — C2), `AnthropicRequestMapper` + DTOs (no sampling params — *structural*, the fields don't exist; one `cache_control` ephemeral on the system block; serialized with `explicitNulls=false`), `ToolUseXmlSerializer` (round-trip-pinned against the real parser; collision-guarded — `error()`s rather than emit a truncating payload), `AnthropicSseParser` (text/`thinking_delta`→`<thinking>`/`tool_use`→XML at `message_stop`), `AnthropicNativeProtocol` (`presentTools→null`, `requiresDialectGuard=false`, `parseToolCalls`→`AssistantMessageParser`, `classifyStreamLine`/`classifyHttpError`), `AnthropicHttpTransport`/`AnthropicHttpClient` (SSE streaming, error→`ApiResult.Error` mapping, debug dumps with redacted key, coroutine-cancellation→`call.cancel()`). The brain bridges the synchronous whole-sequence parser to suspend `onChunk` via a producer/consumer (blocking line-queue + UNLIMITED channel) so text streams **progressively**. **Provider exclusivity** — every provider-dependent site branches on the canonical literal `"anthropic"`: `BrainFactory.create` (native branch FIRST, before the blank-Sourcegraph guard; `modelOverride ?: anthropicModel ?: AnthropicModelCatalog.defaultModel()`), skip `wrapBrainWithRouter` (images go inline, not routed to `/stream`), the `ModelCatalogService` selection, sub-agent default model, OFFLINE probe URL (`anthropicApiUrl`), L2 fallback chain, `availableModels`/in-chat picker, and — critically (C1) — a **per-task provider-selected `ToolProtocol` threaded to all five prompt-build + dialect-guard sites** (orchestrator field, loop facade, resume redaction, `SubagentRunner`, `MessageStateHandler`) so native sub-agents get `presentTools→null` and never double-present tools. **Deferred to 4b:** structured-persistence path (`protocol="native"`, `toChatMessage` branch), `BrainRouter` dissolution, `brain→LlmProvider`, multi-breakpoint prompt caching. **Phase-5 (OSS) flag:** the native transport's prompt-stop uses `@OptIn(InternalCoroutinesApi)` `invokeOnCompletion(onCancelling=true)` — swap for a stable watchdog before publish. Spec: `docs/superpowers/specs/2026-06-30-plugin-split-phase4a-design.md`.
