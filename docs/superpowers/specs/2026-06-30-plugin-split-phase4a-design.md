...
- It makes the persistence back-compat risk (the design's sharpest) **literally zero**: nothing on disk changes (`ApiMessage.protocol` stays `null`, `toChatMessage` keeps its existing XML render at `ApiMessage.kt:114`, `toolCalls` stays `null`), so 4a drops the discriminator branch, the `toApiMessage` change, the protocol-threading into the hardcoded `XmlToolProtocol()` sites, and the mixed-session round-trip tests — ~a third of the surface, the riskiest third. The drift machinery stays armed-but-harmless (canonical XML never trips it).
...
Under Option A, the brain renders to XML internally, so 4a does **not** need the structured protocol on the loop's parse path. To keep `BrainFactory` honest and 4b cheap, the native brain pairs with the existing **`XmlToolProtocol`** (canonical XML never drifts → the drift machinery is inert with no false positives, and no protocol threading into the hardcoded sites is needed). `AnthropicNativeProtocol`'s `classifyStreamLine`/`classifyHttpError` are implemented and called by the brain's SSE/HTTP layer (§5.6); its structured `parseToolCalls` lands in 4b.
...
- `AnthropicModelCatalog` — a **dead-simple static map** of the 4–5 known models → context window / capabilities / status, in the `ModelCatalogService` DTO shape, keyed on the bare model id (no holder refactor). Feeds `ContextManager.getContextWindow`/`effectiveMaxInputTokens` for the 88% compaction gate. Bypass `ModelCache.getModels()` when provider == anthropic-direct.
...
- **(OFFLINE probe) I1:** `llmProbeUrl` is hardcoded to `sourcegraphUrl` (`AgentService.kt:2605`). Branch on provider → `ConnectionSettings.anthropicBaseUrl` so the OFFLINE fail-fast probes the right host (else it's disabled/wrong on native).
...
- **(API-debug parity) M2:** wire `writeApiDebugDumps` tracing into the native client (the Sourcegraph path uses `RawApiTraceInterceptor`) so enabling dumps captures native calls; else document the parity gap.

### 5.8 Settings + factory
- `ConnectionSettings` — `anthropicBaseUrl` (default `https://api.anthropic.com`).
- `AgentSettings` — `llmProvider: String = "sourcegraph"` (`"sourcegraph"`|`"anthropic-direct"`), `anthropicModel: String? = "claude-opus-4-8"`, `anthropicEffort: String? = "high"`, `anthropicThinkingEnabled: Boolean`.
- `CredentialStore` — new `ServiceType.ANTHROPIC` (`x-api-key`); never in XML.
- `BrainFactory.create()` — branch on `llmProvider`: sourcegraph → existing; anthropic-direct → `AnthropicDirectBrain` + `XmlToolProtocol` + `AnthropicModelCatalog`, and (per §5.5) the caller skips `wrapBrainWithRouter`. Provider/model/effort/thinking + the key in Settings → Tools → Workflow Orchestrator → AI Agent.
