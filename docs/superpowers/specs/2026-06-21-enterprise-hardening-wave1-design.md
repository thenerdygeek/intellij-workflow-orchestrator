...
| ~~**C5 — CycloneDX SBOM plugin**~~ **DONE** in follow-up (commit `1c55894ca`) | Applied `org.cyclonedx.bom` 3.2.4 at the root; `cyclonedxBom` aggregates all submodules → `build/reports/cyclonedx/bom.json` (CycloneDX 1.6, 410 components). The strict `verify-metadata=true` reconciliation added +1096 SHA-256 entries (purely additive); verified `cyclonedxBom` passes under strict verification and `verifyPlugin` is unaffected. `release.yml` SBOM step is now non-optional. |
