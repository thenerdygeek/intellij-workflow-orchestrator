...
When tier 1 returns LOW confidence (`GenericIdle` or a weak-signal `LikelyStdinPrompt`) **and** `idle_timeout ≥ 5s` **and** the user has not disabled it, send the last ~40 lines to Haiku with a tight prompt asking `waiting_for_stdin | running_slow | stuck | unknown` plus a confidence score and reason. Replace the tier 1 label with the LLM's verdict. Cache by tail-output hash. Hard timeout 3s; on timeout, fall back to tier 1 label. Not in v1.

**Settings — v1 adds none.** Classification is always on via tier 1. v1.1 will add `idleClassifierMode`, `idleClassifierModel`, `idleClassifierCacheEnabled`, `idleClassifierTimeoutMs`, `idleClassifierSkipBelowIdleSeconds=5`.
...
- Badge in chat input area: "⚙ {N} background" when N>0, clickable to open a popover listing bgIds with kill buttons.
...
- **Tier 2 Haiku idle classifier** — LLM-based classification when tier 1 returns LOW confidence. Adds `idleClassifierMode`, `idleClassifierModel`, `idleClassifierCacheEnabled`, `idleClassifierTimeoutMs`, `idleClassifierSkipBelowIdleSeconds` settings. Gated escalation from regex → LLM → fallback.
