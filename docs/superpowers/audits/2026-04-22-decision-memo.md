...
| `bitbucket_pr.merge_pr` | F-MED | Tool schema description at `BitbucketPrTool.kt:64` lists `"merge-commit, squash, ff-only"` but DC accepts `"no-ff, squash, rebase-no-ff"` (details §`merge_pr` Diff bullet "Strategy enum mismatch", lines 820-821). Per task prompt explicit guidance: description-string mismatches are "borderline ... the actual API call works if you pass the right strategy". Classified F-MED. |
...
- Add pagination loop to `getPullRequestChanges`: add `nextPageStart: Int? = null` to `BitbucketPrChangesResponse` (`BitbucketBranchClient.kt:239-242`), add `start: Int = 0` to `BitbucketBranchClient.getPullRequestChanges` signature at line 1500 and append `&start=$start` to the URL at line 1509, implement the loop until `isLastPage=true` with a max-pages guard (details Notes 1, lines 1280-1284).
...
- Add pagination loop to `getPullRequestActivities`: add `nextPageStart: Int? = null` to `BitbucketPrActivityResponse` (`BitbucketBranchClient.kt:233-236`), add `start: Int = 0` to `BitbucketBranchClient.getPullRequestActivities` at line 1125, implement the loop with a max-pages guard of ~4 pages / 200 activities for LLM-injection safety (details Notes 1, lines 1361-1365).
- Add pagination loop to `getPullRequestCommits`: add `start: Int = 0` to `BitbucketBranchClient.getPullRequestCommits` signature at lines 1539-1543, append `&start=$start` to the URL at line 1549, implement the loop in `BitbucketServiceImpl.getPullRequestCommits` starting at line 147 (`result.data.isLastPage` + `result.data.nextPageStart` are already on the response DTO) (details Notes 1, lines 1438-1457).
